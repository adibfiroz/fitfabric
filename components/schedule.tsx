        ...
        <div className=" relative">
            ...
            <div className='bg-white custom-shadow rounded-2xl p-4 mb-6 sm:pl-48'>
                {clubs.map((club, index) => (
                    <div key={index} className={cn("bg-[#b8df4b1a] rounded-xl md:rounded-full px-4 md:px-6 py-3 text-sm grid grid-cols-12 gap-3 items-center", clubs.length > 1 && "mb-4")}>

                        <div className="font-semibold text-[12px] mb-1 md:mb-0 col-span-12 md:col-span-3">{club.name}</div>
                        <div className="text-gray-600 text-[13px] mb-2 md:mb-0 col-span-12 md:col-span-4">{club.address}</div>
    ...
    return (
        <Tabs defaultValue="list" className="">
            <div className=' flex justify-center items-center flex-wrap gap-5 sm:gap-20 mb-8'>
                <select name="city" id="city" className='custom-select appearance-none w-44 outline-none border-2 border-[#6cce40] rounded-full py-1.5 px-5 text-sm font-semibold' onChange={handleCityChange}>
                ...
                <TabsList className='rounded-full'>
                    <TabsTrigger className=' rounded-full schedule' value="map">
                        <MapPin size={18} className=' mr-1' />
                        Map
                    </TabsTrigger>
                    <TabsTrigger className=' rounded-full schedule' value="list">
